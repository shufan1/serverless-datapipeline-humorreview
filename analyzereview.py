...
def apply_sentiment(df, column="question"):
...
def lambda_handler(event, context):
    ...
    for record in event["Records"]:
        ...
        extra_logging = {"body": body, "question": question,"label":label,"product":product}
        LOG.info(
    ...
    LOG.info(f"Creating dataframe with values: {review_question}")
    df = pd.dataframe({"product":products,"label":label,"question":review_questions,})

    # Perform Sentiment Analysis
    df = apply_sentiment(df)
    LOG.info(f"Sentiment from Amazon reviews: {df.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="reviewsentiment", name=names)
